...
# 전역 변수로 Qwen3 모델 초기화를 지연
_qwen3_tokenizer = None
_qwen3_model = None
_qwen3_yes_token = None
_qwen3_no_token = None

def get_qwen3_model():
    """Qwen3 모델을 lazy loading으로 가져오기 (thread-safe)"""
    global _qwen3_tokenizer, _qwen3_model, _qwen3_yes_token, _qwen3_no_token
    if _qwen3_model is None:
        with _model_lock:  # 한 번에 하나의 스레드만 모델 로드
            if _qwen3_model is None:  # double-check locking pattern
                print(">> Qwen3 모델 로드 시작...")
                import time
                start_time = time.time()
                try:
                    _qwen3_tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-Reranker-0.6B", padding_side="left")
                    ...
                    # yes/no 토큰 id 계산
                    _qwen3_yes_token = _qwen3_tokenizer.convert_tokens_to_ids("yes")
                    _qwen3_no_token = _qwen3_tokenizer.convert_tokens_to_ids("no")
                    
                    elapsed = time.time() - start_time
                    print(f">> Qwen3 모델 로드 성공 (소요시간: {elapsed:.2f}초)")
                except Exception as e:
                    print(f"!! Qwen3 모델 로드 실패: {e}")
                    raise
    return _qwen3_tokenizer, _qwen3_model, _qwen3_yes_token, _qwen3_no_token

# Qwen3 모델은 get_qwen3_model() 함수를 통해 lazy loading됩니다

# 모델 카드의 템플릿을 그대로 사용 (Instruction Aware)
_QWEN3_SYSTEM = (
...
_QWEN3_MAXLEN = 8192
_QWEN3_DEFAULT_INSTRUCT = "Given a web search query, retrieve relevant passages that answer the query"

def _qwen3_format(instruction: str, query: str, doc: str) -> str:
    return (
...
@torch.no_grad()
def _qwen3_score_pairs(pairs, instruction: str = None):
    """pairs: List[Tuple(query, doc)] → List[score in 0..1]"""
    # Qwen3 모델 lazy loading
    tokenizer, model, yes_token, no_token = get_qwen3_model()
    
    instruction = instruction or _QWEN3_DEFAULT_INSTRUCT
    # 템플릿 적용
    prompts = [_qwen3_format(instruction, q, d) for q, d in pairs]
    # 토크나이즈(+ suffix)
    prompts = [p + _QWEN3_SUFFIX for p in prompts]  # suffix까지 문자열로 합쳐서
    toks = tokenizer(
        prompts,
        padding=True,           # 또는 'max_length'
        truncation=True,
        max_length=_QWEN3_MAXLEN,
        return_tensors="pt"
    )
    for key in toks:
        toks[key] = toks[key].to(model.device)
    
    # 마지막 토큰의 로짓에서 yes/no 확률 계산
    out = model(**toks)
    batch_scores = out.logits[:, -1, :]                            # (B, vocab)
    true_vector = batch_scores[:, yes_token]
    false_vector  = batch_scores[:, no_token]
    batch_scores = torch.stack([false_vector, true_vector], dim=1)
    batch_scores = torch.nn.functional.log_softmax(batch_scores, dim=1)
    probs_yes = batch_scores[:, 1].exp().tolist()
    return probs_yes  # 0..1
...
"""
    ...
    def sparse_retrieval_index(self, query: str, index: str, top_k: int = 20) -> List[Dict]:
    ...
    def BGE_rerank(self, results, query, top_k=20):
        ...
        try:
            ...
            print(f"BGE-rerank 오류: {e}")
            # 오류 발생시 원본 결과 반환
            return subset

    
    def qwen3_reranking(self, results: List[Dict], query: str, top_k: int = 20,
                    instruction: str = None) -> List[Dict]:
        """
        Qwen/Qwen3-Reranker-4B 기반 리랭킹 (로컬/HF)
        - outputs: 각 item에 rerank_score(0..1), rerank_rank 부여
        """
        if not results:
            return results
        subset = results[:top_k]
        ...
        try:
            ...
            scores = list()
            # for pair in pairs:
            #     score = _qwen3_score_pairs([pair])
            #     scores.append(score[0])
            batch_size = 3
            for i in range(0, len(pairs), batch_size):
                batch = pairs[i:i + batch_size]  # 마지막 남은 것(<3)도 한 번에 처리
                batch_scores = _qwen3_score_pairs(batch)  # len(batch) 길이의 점수 리스트 반환 가정
                # 점수들을 float으로 강제 변환(텐서/넘파이 호환)
                scores.extend([float(s) for s in batch_scores])
            print(scores)
            reranked = []   
            for r, s in zip(subset, scores):
                item = r.copy()
                item["rerank_score"] = float(s)  # 0..1
                reranked.append(item)
